...
legacy_rel = mf.get("segment_ui") or ""
...
if legacy_rel:
    ...
    if p and p.exists():
        ...
        st.set_page_config(page_title="40 • Build Luminaire", layout="wide")
        st.error(f"Legacy UI not found: {legacy_rel}")
else:
    import streamlit as st
    st.set_page_config(page_title="40 • Build Luminaire", layout="wide")
    st.info("Set `segment_ui` in manifest.yml (path inside legacy_src)")
